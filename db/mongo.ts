...
export async function mongodb(srv: string, dbName: string, collectionName: string): Promise<DB> {
    ...
    return {
        ...
        async set(namespace, data) {
            const result = await collection.updateOne({ namespace }, { $set: { data } }, { upsert: true });
